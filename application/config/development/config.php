...
if (isset($GLOBALS['config_file'])) {
    ...
    // fallback, make it work as it used to
    if (isset($_SERVER['HTTP_HOST'])) {
            if(preg_match('/(www|dev[0-9]+)\.androidmakeup\.(com|local)/', $_SERVER['HTTP_HOST'], $matches)) {
                    define("DEVELOPER", $matches[1] == "www" ? "DEV0" : strtoupper($matches[1]));
                    $config['base_url']	    = 'http://'.$matches[0];
            } else {
                    define("DEVELOPER", "DEV0");		
                    $config['base_url']	    = 'http://'.$_SERVER['HTTP_HOST'];
            }

    } else {
            $host = gethostname();
            if (preg_match('@(www|dev[0-9]+)@', gethostname(), $matches)) {
                    define("DEVELOPER", $matches[1] == "www" ? "DEV0" : strtoupper($matches[1]));
                    $config['base_url'] = 'http://' . $matches[1] . 'androidmakeup.com';
            } else {
                    define("DEVELOPER", "DEV0");
                    $config['base_url'] = 'http://dev0.androidmakeup.local';
            }

    }
    
    define('KMSPATH', '/www/CIKMS/KMS');
    $config['cookie_domain']	= "." . trim(preg_replace("@(http://)?(www|dev[0-9]+)?@","",$config['base_url']),'.');
}
...
$assets_domain = isset($config['assets_url']) ? $config['assets_url'] : $config['base_url'] .'/assets';

$config['assets']['js_url']    = $assets_domain;
$config['assets']['css_url']   = $assets_domain;
$config['assets']['less_url']  = $assets_domain;
$config['assets']['img_url']   = $assets_domain;
