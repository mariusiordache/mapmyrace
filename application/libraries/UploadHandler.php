...
class UploadHandler {
    ...
    function __construct($config = array('options' => null, 'initialize' => true, 'error_messages' => null)) {
        extract($config);
        $this->options = array(
    ...
    protected function validate($uploaded_file, &$file, $error, $index) {
        ...
        $resource = get_resource_for_file($file->name);
        
        if (!empty($resource['resource'])) {
            $type = !empty($resource['resource']['type']) ?  $resource['resource']['type'] : 'image';
            $extensions = null;
            switch($type) {
                case 'image':
            ...
            if ($extensions && !preg_match("@\.({$extensions})$@i", $file->name)) {
                $file->error = $this->get_error_message('accept_file_types') . " for {$type} resources";
                return false;
            }
        }
    ...
    protected function handle_form_data($file, $index) {
        ...
        if (empty($this->options['overwrite_upload_dir'])) {
            // get_resource_folder is a global function defined in upload.php		
            $subfolder = isset(get_instance()->globals['force_subfolder']) ? get_instance()->globals['force_subfolder'] : get_resource_folder($file->name);

            $this->options['upload_dir'] = get_instance()->globals['current_theme']->get_resources_path() . '/' . $subfolder . '/';
            $this->options['upload_url'] = get_instance()->globals['current_theme']->get_resources_url() . '/' . $subfolder . '/';

            $this->options['backup_dir'] = $this->options['changeSetId'] ? get_instance()->globals['current_theme']->get_change_set_path($this->options['changeSetId']) . '/' . $subfolder . '/' : null;
        } else {
            $this->options['upload_dir'] = $this->options['overwrite_upload_dir'];
        }
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if (!is_dir($upload_dir)) {
                ...
                chmod($upload_dir, $this->options['mkdir_mode']);
            }

            $file_path = $this->get_upload_path($file->name);
            $backup_dir = isset($this->options['backup_dir']) ? $this->options['backup_dir'] : null;
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    // we need to backup the old file to the specified changeset
                    // old file may not have the same extension as the current file
                    // so we need to search it first
                    $old_resources = get_similar_resources($file_path);
                    $old_file_path = !empty($old_resources) ? array_shift($old_resources) : $file_path;

                    if ($old_file_path && file_exists($old_file_path) && $backup_dir) {
                        if (!is_dir($backup_dir)) {
                            mkdir($backup_dir, $this->options['mkdir_mode'], true);
                            chmod($backup_dir, $this->options['mkdir_mode']);
                        }
                        $oldfilename = trim(str_replace(dirname($old_file_path), "", $old_file_path), "/");
                        copy($old_file_path, $backup_dir . "/" . $oldfilename);
                        @unlink($file_path);
                    }
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                ...
                if (!empty($_POST['resource_id']) && count($upload['tmp_name']) == 1) {
                    if (preg_match("@\.(jpg|jpeg|9\.png|png|ttf|otf|wav|mp3)$@i", $upload['name'][$index], $m)) {

                        $upload['name'][$index] = $_POST['resource_id'] . '.' . $m[1];
                    }
                }
