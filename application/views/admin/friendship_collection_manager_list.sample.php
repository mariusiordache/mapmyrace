...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="request_user_id" <?php if($search_field=='request_user_id') { echo 'selected="selected"'; } ?> >request_user_id</option><option value="target_user_id" <?php if($search_field=='target_user_id') { echo 'selected="selected"'; } ?> >target_user_id</option><option value="accepted" <?php if($search_field=='accepted') { echo 'selected="selected"'; } ?> >accepted</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="request_user_id" <?php if($sort_field=='request_user_id') { echo 'selected="selected"'; } ?> >request_user_id</option><option value="target_user_id" <?php if($sort_field=='target_user_id') { echo 'selected="selected"'; } ?> >target_user_id</option><option value="accepted" <?php if($sort_field=='accepted') { echo 'selected="selected"'; } ?> >accepted</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>request_user_id</th><th>target_user_id</th><th>accepted</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['request_user_id']['admin_header'] == 'on') { echo '<td>'.$item['request_user_id'].'</td>';  } ?><?php if($fields['target_user_id']['admin_header'] == 'on') { echo '<td>'.$item['target_user_id'].'</td>';  } ?><?php if($fields['accepted']['admin_header'] == 'on') { echo '<td>'.$item['accepted'].'</td>';  } ?>					<td>	
